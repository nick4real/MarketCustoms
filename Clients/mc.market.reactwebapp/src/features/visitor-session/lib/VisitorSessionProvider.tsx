...
import { ensureCurrentUserMetadata } from "@/entities/profile";
import { mapAccount, type VisitorSessionView } from "@/entities/session";
import { isAuth0Configured } from "./auth0";
import { VisitorSessionContext } from "./visitorSessionContext";
import { type ProfileLoadState, toVisitorSession } from "./visitorSessionView";
    ...
    void (async () => {
      try {
        ...
        const metadata = await ensureCurrentUserMetadata(
          token,
          {
            displayName: account.displayName ?? "New member",
